...
- **Dedupe on import: key on slug, not raw URL** (see the normalization note above) — re-importing or overlapping rows should upsert against the existing problem rather than create a duplicate, **unless that problem already has real review progress beyond its own creation** (more than one logged attempt — whether from honest re-grading via the due page, or an earlier bulk import that was itself later refreshed). In that case the row is left completely untouched rather than upserted: re-pasting an old CSV export months later must not silently reset honestly-earned progress (e.g. a problem you've leveled up to Good with a multi-week interval) back down to a fresh baseline Hard grade across however many rows happen to match. This is a deliberate amendment to the single-add rule directly above — a single-add re-grade is one row a user consciously chose to re-grade; a bulk-import upsert is an incidental side effect that could hit an entire pasted batch at once, so it gets a safety check the single-add path doesn't need.
...
- **Stagger initial `next_review_date` by import order rather than defaulting every row to the same date** — without this, all imported problems become "overdue" simultaneously, producing a pile-up on day one that directly contradicts the adaptive-workload design in §4. The spread window is sized from a target new-reviews-per-day figure (an import-time input, default 5, floored at a 14-day minimum spread) rather than a fixed ~2-week window — a fixed window doesn't scale: a 180-problem backlog at a fixed 14 days works out to ~13/day, which is exactly the kind of pile-up this rule exists to prevent for a larger import.
